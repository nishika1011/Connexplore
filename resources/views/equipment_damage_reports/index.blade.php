...
<body>
    <div class="container">
        <h1 class="text-3xl font-bold mb-4 text-white">All Damage Reports</h1>
        ...
        <table class="table table-bordered w-full">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="px-4 py-2">{{ $report->incident_time }}</td>
                    ...
                    <td class="px-4 py-2">
                        @if($report->images)
                            <img src="{{ asset('storage/' . $report->images) }}" alt="Damage" width="100" class="rounded shadow">
                        @endif
                    </td>
                    <td class="px-4 py-2">
                        <!-- Form to update status -->
                        <form method="POST" action="{{ route('damage-reports.update-status', $report->id) }}">
                            @csrf
                            @method('PUT')

                            <select name="status" class="border rounded px-2 py-1" onchange="this.form.submit()">
                                <option value="">Select</option>
                                <option value="Actions taken" {{ $report->status === 'Actions taken' ? 'selected' : '' }}>Actions taken</option>
                                <option value="Under investigation" {{ $report->status === 'Under investigation' ? 'selected' : '' }}>Under investigation</option>
                                <option value="Incident reported" {{ $report->status === 'Incident reported' ? 'selected' : '' }}>Incident reported</option>
                            </select>
                        </form>
                    </td>
                </tr>
