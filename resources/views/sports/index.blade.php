...
<body class="bg-gray-100">
    ...
    <table class="table-auto w-full max-w-4xl mx-auto mt-4 border-collapse text-black">
        <thead class="bg-blue-600 text-white">
            <tr>
                ...
                <th class="px-4 py-2">Max Players</th>
                <th class="px-4 py-2">Min Players</th>
        ...
        <tbody class="overflow-y-auto max-h-72">
            @foreach ($sports as $sport)
                <tr class="border-b bg-white hover:bg-gray-100">
                    <td class="px-4 py-2 text-black">{{ $sport->name }}</td>
                    <td class="px-4 py-2 text-black">{{ $sport->max_players }}</td>
                    <td class="px-4 py-2 text-black">{{ $sport->min_players }}</td>
                    <td class="px-4 py-2 text-black">
                        {{ $sport->location ? $sport->location->branch : 'No location' }}
                    </td>
                    <td class="px-4 py-2 text-black">
                        @if ($sport->image)
                            <img src="{{ asset('storage/' . $sport->image) }}" alt="{{ $sport->name }}" width="100" class="rounded">
                        @else
                            No image
                        @endif
                    </td>
                    <td class="px-4 py-2 flex space-x-2 text-black">
                        <!-- Edit Button -->
                        <a href="{{ route('sports.edit', $sport->id) }}" title="Edit" class="text-xl text-yellow-600 hover:text-yellow-700">
                            ✏️
                        </a>
                        <!-- Delete Button -->
                        <form action="{{ route('sports.destroy', $sport->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" title="Delete" class="text-xl text-red-600 hover:text-red-700" onclick="return confirm('Are you sure you want to delete this sport?')">
                                ❌
                            </button>
                        </form>
                    </td>
    ...
    <div class="flex justify-end space-x-4 mt-4">
        <!-- Add Sports Button -->
        <a href="{{ route('sports.create') }}" class="bg-blue-800 hover:bg-blue-900 text-white font-bold py-2 px-4 rounded-lg shadow-md">
            Add Sports
        </a>
