...
<body class="flex flex-col flex-1 bg-gray-900 text-gray-100">
    ...
    <main class="container mx-auto p-4">
        <h1 class="text-3xl font-bold mb-6 text-center">Schedule Your Game Now!</h1>

        {{-- Success message --}}
        @if (session('success'))
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-6 max-w-xl mx-auto text-center">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach ($sports as $sport)
                <div class="bg-gray-700 rounded-lg overflow-hidden shadow-lg">
                    <img src="{{ asset('storage/' . $sport->image) }}" alt="{{ $sport->name }}" class="w-full h-48 object-cover">
                    <div class="p-4">
                        <h3 class="text-xl font-semibold text-white">{{ $sport->name }}</h3>
                        <p class="text-gray-300">{{ $sport->description }}</p>
                        <br>
